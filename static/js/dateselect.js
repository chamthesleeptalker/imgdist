...
var navPrev = svg.append("g")
    ...
    .on('click', function() {
      start_date = start_date.subtract(vis.navjump,'d').startOf();
...
navPrev.append("rect")
    .attr('height', vis.height + 26 -12.5)
...
var navNext = svg.append("g")
    ...
    .on('click', function() {
      end_date.add(vis.navjump,'d');
      end_date = end_date.endOf().unix() > moment().endOf().unix() ? moment().endOf() : end_date;
...
function renderVis(dir) {
  ...
  while (end_date.diff(current) >= 0) {
    ...
    data.push(datum);
    current = current.add(1,'d').startOf();
  }
...
/*       .attr('transform', function(d) {return 'translate('+(x(d.values[0].date))+','+(vis.height + 12.5)+')'}) */
  ...
  var wgrp_enter = weekgroup.enter().insert('g')
      ...
      .attr('transform', function(d) {return 'translate('+(x(d3.time.day.offset(d.values[0].date, dir*vis.navjump)))+','+(vis.height)+')'})
  wgrp_enter.transition()
      //.attr('transform', function(d) {return 'translate('+(x(d.values[0].date))+','+(vis.height + 12.5)+')'})
      .attr('transform', function(d) {return 'translate('+(x(d.values[0].date))+','+(vis.height)+')'})
      .attr('opacity', 1)
  ...
  navNext.transition()
    .attr('opacity', end_date.unix() == moment().endOf().unix() ? 0 : 0.4)
    .attr('display', end_date.unix() == moment().endOf().unix() ? 'none' : 'block')
...
function brushmove(extentSelected) {
  var g = d3.select(this.parentNode);
  if(extentSelected){
    selection = extentSelected;
    extent = extentSelected;
